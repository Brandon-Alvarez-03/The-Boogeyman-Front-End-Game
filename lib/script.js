...
// NEXT STEPS: Establish the player class for Michael by copying and modifying Laurie's Class
// Have him spawn from off left side of screen
// have him creep up as time goes on
// fix timer display to go above one minute
// dont forget to un-comment if conditions in the animate section
// fix obstacle widths for collision
// start it on laurie's lookback somehow
// might want to switch to an extyended version of the shape hunts allyson
...
//could instantiate everything inside an event handler after declaring undefined global variables for each isntantiation
...
window.addEventListener("load", () => {
  ...
  class MichaelMeyers {
    ...
    update(deltaTime, obstacles) {
      ...
      if (laurie.x <= this.x + 20) {
        laurie.x = this.x + 20; // if player position moves onto neg x-axis set player position back to zero (left game boundary)
      }
      if (dist < collideDist) {
        gameLost = true;
        gameOver = true;
  ...
  class LaurieStrode {
    ...
    update(input, deltaTime, obstacles) {
      ...
      obstacles.forEach((obstacle) => {
        ...
        const dist = Math.sqrt(dx ** 2 + dy ** 2); //pythagorean theorem
        // console.log(dist);
        if (dist < obstacle.width / 2 + this.width / 2) {
          //160 px
          this.x -= 100;
        }
      });
  ...
  class Obstacle {
    constructor(gameWidth, gameHeight) {
      ...
      this.speed = 8; // rate at which obstacles approach Laurie // could possibly have difficulty where its random and another whhere it's constant
      this.toDelete = false;
    }
    draw(context) {
  ...
  let randomSpawnInterval = Math.random() * 1000 + 600;
